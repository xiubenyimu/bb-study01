...
public static int mark_find(String possible_word)
{
  ...
  return pos;
	...
	public String genFormData(String book,String chapter_no,String verse_no,String word_tag_no,String verse,String reading_type)
	{
		String phrase=verse;
		String phrase_meaning="";
		String ret_v="";
		ret_v+="<form action=\"OutputWordsInsideChapter\" method=\"POST\">";
		ret_v+="<br><b>Phrase</b><br>";
		ret_v+="<textarea name =phrase rows=\"10\" cols=\"90\">"+phrase+"</textarea>";
		ret_v+="<br><b>Meaning</b><br>";
		ret_v+="<textarea name =phrase_meaning rows=\"10\" cols=\"90\">"+phrase_meaning+"</textarea>";
		ret_v+="<input type=\"hidden\" name=\"book\" value=\""+book+"\" />";
		ret_v+="<input type=\"hidden\" name=\"chapter_no\" value=\""+chapter_no+"\" />";
		//ret_v+="<input type=\"hidden\" name=\"mo_word\" value=\""+"null"+"\" />";
		//ret_v+="<input type=\"hidden\" name=\"mo_meaning\" value=\""+"null"+"\" />";
            ...
            throws ServletException, IOException
  {
	...
	String word_tag_no=request.getParameter("word_tag");
	...
	String docType =
      ...
      out.println(docType +
                ...
                "<br><br>Insert a piece of PHRASE info around the word <b>"+word+"</b> blow"+
...
//out.println("After DriverManager.getConnection<br>");
         ...
         //String sql="SELECT question_content,answer FROM "+question_answer_tbname+" WHERE book='"+book+"' AND chapter_no="+chapter_no+" AND verse_no="+verse_no;
         String sql="SELECT verse FROM "+verse_info_tbname+" WHERE book='"+book+"' AND chapter_no="+chapter_no+" AND verse_no="+verse_no;
         //out.println(sql);
...
//out.println("wQuestion_tag="+wQuestion_tag+"<br>");
/*
	...
	out.println("<input type=\"hidden\" name=\"verse_tag\" value=\"verse_no_"+"null"+"\" />");
	out.println("<input type=\"hidden\" name=\"word_tag\" value=\""+"null"+"\" />");
	out.println("<input type=\"hidden\" name=\"wQuestion_tag\" value=\""+wQuestion_tag+"\" />");
	out.println("<input type=\"submit\" value=\"UPDATE\" />");
	out.println("</form>");
	*/
	
	String formData=genFormData(book,chapter_no,verse_no,word_tag_no,verse,reading_type);
